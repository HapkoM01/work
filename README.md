...
Добавлен новый модуль `decorators.py`, содержащий декоратор `log` для автоматического логирования выполнения функций.

### Декоратор `log`

Декоратор `log` автоматически записывает информацию о вызовах функций:

- Время вызова (неявно через запись в момент вызова)
- Имя функции
