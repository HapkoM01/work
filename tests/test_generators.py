...
def sample_transactions() -> List[Dict[str, Any]]:
    """Фикстура с тестовыми транзакциями (4 записи, разные валюты и описания)."""
    return [
        {
            "id": 939719570,
            "state": "EXECUTED",
...
def transactions_without_currency() -> List[Dict[str, Any]]:
    """Фикстура с транзакциями без информации о валюте или с некорректной структурой."""
    return [
        {"id": 1, "description": "Транзакция 1"},
        {"id": 2, "operationAmount": {"amount": "100"}},
        {"id": 3, "operationAmount": {"amount": "200", "currency": {}}},
        {"id": 4, "operationAmount": {"currency": {"code": "USD"}}},  # нет amount
    ]
...
class TestFilterByCurrency:
    ...
    def test_filter_usd_transactions(self, sample_transactions):
        """Фильтрация по USD возвращает правильные транзакции."""
    ...
    def test_filter_rub_transactions(self, sample_transactions):
        """Фильтрация по RUB возвращает одну транзакцию."""
    ...
    def test_filter_nonexistent_currency(self, sample_transactions):
        """Фильтрация по несуществующей валюте возвращает пустой итератор."""
    ...
    def test_transactions_without_currency(self, transactions_without_currency):
        """Транзакции без полной структуры, но с currency.code — могут быть включены."""
    ...
    def test_invalid_currency_code_raises_error(self, sample_transactions):
        """Пустой код валюты вызывает ValueError."""
...
class TestTransactionDescriptions:
    ...
    def test_all_descriptions(self, sample_transactions):
        """Все описания — строки, длина совпадает с входом."""
        descriptions = list(transaction_descriptions(sample_transactions))
    ...
    def test_transactions_without_description(self):
        """Отсутствие описания → пустая строка."""
        transactions = [
            {"id": 1},
...
class TestCardNumberGenerator:
    ...
    @pytest.mark.parametrize(
        ...
        [(1, 5, 4), (10, 15, 5), (1, 1, 0), (9999999999999990, 9999999999999999, 9)],
    )
    def test_range_length(self, start, stop, expected_count):
        """Правильное количество номеров в диапазоне."""
    ...
    def test_card_format(self, number, expected_format):
        """Правильный формат номера карты (16 цифр, пробелы каждые 4)."""
        cards = list(card_number_generator(number, number + 1))
        assert len(cards) == 1
        assert cards[0] == expected_format

    def test_sequential_numbers(self):
        """Последовательные номера генерируются правильно."""
    ...
    def test_large_range(self):
        """Генерация в большом диапазоне (около максимума)."""
    ...
    def test_generator_behavior(self):
        """Проверка итератора и StopIteration."""
